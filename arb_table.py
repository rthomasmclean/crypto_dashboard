...
class table:
    ...
    def update_table(self, name, exchange, borrow, deposit, tvl):
        entry = {"Exchange": exchange, "Cost to Borrow": borrow, 
                 "Yield": deposit, "TVL": tvl}
        # non-empty key
        ...
        # Replace data when exchange is already in key
             ...
             (self.dict[name][self.dict[name]["Exchange"] == exchange] \
             .empty == False)):
            index = self.dict[name] \
                        .index[self.dict[name]["Exchange"] == exchange]
            self.dict[name].loc[index] = [exchange, borrow, deposit, tvl]
        # First row in previously empty DataFrame
        else:
            self.dict[name] = pd.DataFrame(entry, index=[0])
    ...
    def best_spread(self):
        ...
        for key in self.dict.keys():
            ...
            max_exchange = self.dict[key]["Exchange"] \
                           [self.dict[key]["Yield"] == max_yield].values
            max_tvl = self.dict[key]["TVL"] \
                      [self.dict[key]["Yield"] == max_yield].values
            ...
            min_exchange = self.dict[key]["Exchange"] \
                           [self.dict[key]["Cost to Borrow"] == min_cost] \
                           .values
            min_tvl = self.dict[key]["TVL"][self.dict[key] \
                      ["Cost to Borrow"] == min_cost].values
            # Ensure that value exists to avoid 
            ...
            if (len(min_exchange) > 0):
                ...
                min_exchange = ""
                min_tvl = ""
            cols = ["Max Yield", "Max Yield Exchange", "Max Yield TVL", 
                    "Min Cost", "Min Cost Exchange", "Min Cost TVL", 
                    "Spread", "Color"]
            spread = "-"
            color = ""
            ...
            if (np.isnan(min_cost) == False):
                ...
                min_cost = "-"
            data = [max_yield, max_exchange, max_tvl, 
                    min_cost, min_exchange, min_tvl, 
                    spread, color]
